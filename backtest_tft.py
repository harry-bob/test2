#!/usr/bin/env python3
"""
Backtest the prediction model on Team Formation Test (non-EGOI) contests.
...
# Non-EGOI TFT contests to backtest
tft_contests = ['325', '366', '392', '442']
...
def get_individual_ranks(cid):
    ...
    # Check if this is a TFT contest (use rankings directly)
    is_tft = 'team formation' in c.get('name', '').lower()
    
    participants = {}
    
    if is_tft:
        # For TFT contests, use rankings directly (top 20 = team composition)
        ...
        # For other contests, get individual ranks
        for entry in rankings:
            handle = entry.get('handle', '')
            rank = entry.get('rank_normalized')
            if rank is not None:
...
# Run backtesting
print("="*80)
print("BACKTESTING PREDICTION MODEL ON TEAM FORMATION TEST (Non-EGOI)")
print("="*80)

all_results = []

for i, target_contest_id in enumerate(tft_contests):
